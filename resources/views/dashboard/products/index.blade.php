                    ...
                    @if ($products->count() > 0)
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach ($products as $index=>$product)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{optional($product->categorios)->name}}</td>
                                    <td>{{ $product->name }}</td>
                                    <td><img src="{{ $product->image_path }}" style="width: 100px"  class="img-thumbnail" alt=""></td>
                                    <td>{{ $product->description }}</td>
                                    <td>{{ $product->price }}</td>
                                    {{--<td>{{ $product->tasks->count() }}</td>--}}
                                    <td><a href="{{ route('dashboard.tasks.create', ['product_id' => $product->id]) }}" class="btn btn-info btn-sm">  @lang('site.view')</a></td>       
